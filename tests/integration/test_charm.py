...
async def test_audit_log(ops_test: OpsTest) -> None:
    ...
    audit_log_snap_path = "/var/log/mongodb/audit.log"

    for unit in ops_test.model.applications[mongodb_application_name].units:
        audit_log = subprocess.check_output(
            f"JUJU_MODEL={ops_test.model_full_name} juju ssh  --container mongod {unit.name}  'cat {audit_log_snap_path}'",
...
async def test_log_rotate(ops_test: OpsTest) -> None:
    ...
    audit_log_snap_path = "var/log/mongodb/"

    for unit in ops_test.model.applications[mongodb_application_name].units:
        log_files = subprocess.check_output(
            f"JUJU_MODEL={ops_test.model_full_name} juju ssh  --container mongod {unit.name}  'ls {audit_log_snap_path}'",
            stderr=subprocess.PIPE,
            shell=True,
            universal_newlines=True,
        )
