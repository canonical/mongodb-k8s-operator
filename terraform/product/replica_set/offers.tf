...
resource "juju_offer" "mongodb_client_offer" {
  for_each = var.data_integrator.model != var.mongodb_k8s.model ? { "offered" = true } : {}
...
resource "juju_offer" "tls_provider_offer" {
  for_each = local.enable_tls && var.self_signed_certificates.model != var.mongodb_k8s.model ? { "offered" = true } : {}
...
resource "juju_offer" "s3_integrator_offer" {
  for_each = var.s3_integrator.model != var.mongodb_k8s.model ? { "offered" = true } : {}
